...
一个面向课堂使用的选择题测验网站。名单内学生使用学号和密码登录，旁听生可填写学号和姓名进入；教师可以创建、发布和管理测验。
...
- **MinIO/S3 兼容对象存储**：保存问答图片和课程资料；地址与凭据通过 `.env` 配置
...
- 教师密码登录
- 创建、修改和删除选择题测验
- Quiz 标题、题目和选项支持 LaTeX 公式，并在教师编辑器中实时预览：`$...$` 为行内公式，`$$...$$` 为独立公式
- 每道题可上传一张 JPG、JPEG 或 PNG 图片（最大 5 MB），也支持只用图片作为题目
- 自定义测验代码，留空时自动生成
- 保存草稿、发布测验、将测验下线
- 查看学生姓名、学号、正确题数和提交时间
- 查看公开排行榜
...
- 使用手机或电脑作答
- 每个学号对同一份测验只能提交一次
- 进入答题页后自动开始个人 10 分钟倒计时；刷新或重新登录不会重置时间
- 实时显示“未提交人数 / 已开始人数”，每 30 秒刷新一次
- 至少 5 人开始后，学生进入最后 1 分钟时若未提交人数超过已开始人数的 40%，自动增加 2 分钟；之后每次进入最后 1 分钟都会重新判断
- 倒计时结束且不符合延时条件时自动提交，未作答题目按错误计算
- 提交后查看总分及每题对错
- 提交后用红色题号标记错题，并在总分区域汇总答错题号；不显示标准答案
- 排行榜公开显示姓名，不显示学号
- 提问并可附带图片，查看教师回答
- 下载教师发布的课程资料
...
- 填空题和主观题
- 教师设置统一的开放时间和截止日期（当前仅支持每位学生独立计时）
- 学生自助修改或找回密码
...
预览模式支持学生账号、旁听生入口、名单同步、测验管理、统计、问答和资料上传。数据及测试文件保存在进程内存中，`Ctrl + C` 后会丢失。**正式数据使用 PostgreSQL 和 MinIO/S3 兼容对象存储。**
...
| `MINIO_ENDPOINT` | MinIO/S3 兼容对象存储地址；本机服务通常为 `http://127.0.0.1:9000` |
| `MINIO_ROOT_USER` | 对象存储访问账号 |
| `MINIO_ROOT_PASSWORD` | 对象存储访问密码 |
